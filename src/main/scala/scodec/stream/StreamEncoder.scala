 ...
 */
final class StreamEncoder[A](private val step: StreamEncoder.Step[A]) { self =>
 ...
 /** Converts this encoder to a `Pipe[F, A, Byte]`. */
  ...
  def xmapc[B](f: A => B)(g: B => A): StreamEncoder[B] = new StreamEncoder[B](
    new StreamEncoder.Step[B] {
      def apply[F[_]: RaiseThrowable](s: Stream[F,B]): Pull[F,BitVector,Option[(Stream[F,B], StreamEncoder[B])]] =
