...
package object encode {
   ...
   */
  def tryOnce[A](implicit A: Encoder[A]): StreamEncoder[A] = StreamEncoder {
    P.await1[A].flatMap { a => A.encode(a).fold(
      _ => P.halt,
      P.emit
    )}
